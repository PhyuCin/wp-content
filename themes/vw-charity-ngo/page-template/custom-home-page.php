...
<main id="maincontent" role="main">
  ...
  <?php if( get_theme_mod('vw_charity_ngo_slider_hide_show',true) != ''){ ?>
    <section id="slider">
  ...
  <section id="scholarship">
    <div class="container">
      <div class="row scholarship-box">
        <?php $scholarship_page = array();
          for ( $count = 1; $count <= 3; $count++ ) {
            $mod = intval( get_theme_mod( 'vw_charity_ngo_scholarship_page' . $count ));
            if ( 'page-none-selected' != $mod ) {
              $scholarship_page[] = $mod;
            }
          }
          if( !empty($scholarship_page) ) :
            $args = array(
              'post_type' => 'page',
              'post__in' => $scholarship_page,
              'orderby' => 'post__in'
            );
            $query = new WP_Query( $args );
            if ( $query->have_posts() ) :
      ...
      <div class="container">
        ...
        <div class="row">
          <?php $what_do_page = array();
            ...
            if( !empty($what_do_page) ) :
              $args = array(
                'post_type' => 'page',
                'post__in' => $what_do_page,
                'orderby' => 'post__in'
              );
              $query = new WP_Query( $args );
              if ( $query->have_posts() ) :
